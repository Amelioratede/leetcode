...
int partition(vector<int>& lst,int left, int right){
    int idx=left;
    ...
    swap(lst[idx],lst[left]);
    return idx;


}

void quicksort(vector<int>& lst,int l,int r){
    if(l<r){
        int pivot = partition(lst,l,r);
...
int main(){
    vector<int> lst={5,3,2,6,4,1,8,7};
